...
/**
 * Get effective route number, using override if set, otherwise calculated default
 */
export function getEffectiveRouteNumber(baseNumber: number, spotNumber: number, routeOverride?: number | null): number {
  return routeOverride ?? calculateRouteNumber(baseNumber, spotNumber);
}
